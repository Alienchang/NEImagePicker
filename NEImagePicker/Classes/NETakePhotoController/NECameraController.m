...
@interface NECameraController () <AVCaptureFileOutputRecordingDelegate, UIGestureRecognizerDelegate>
...
@property (nonatomic, copy) void (^didRecordCompletionBlock)(NECameraController *camera, NSURL *outputFileUrl, NSError *error);
@end

NSString *const NESimpleCameraErrorDomain = @"NESimpleCameraErrorDomain";

@implementation NECameraController
...
{
    return [self initWithQuality:AVCaptureSessionPresetHigh position:NECameraPositionRear videoEnabled:videoEnabled];
}
...
- (void)setupWithQuality:(NSString *)quality
                position:(NECameraPosition)position
            videoEnabled:(BOOL)videoEnabled
{
    ...
    _flash = NECameraFlashOff;
    _mirror = NECameraMirrorAuto;
...
- (void)start
{
    [NECameraController requestCameraPermission:^(BOOL granted) {
        if(granted) {
            // request microphone permission if video is enabled
            if(self.videoEnabled) {
                [NECameraController requestMicrophonePermission:^(BOOL granted) {
                    if(granted) {
                        [self initialize];
                    }
                    else {
                        NSError *error = [NSError errorWithDomain:NESimpleCameraErrorDomain
                                                             code:NESimpleCameraErrorCodeMicrophonePermission
                                                         userInfo:nil];
                        [self passError:error];
                    }
                }];
            ...
            NSError *error = [NSError errorWithDomain:NESimpleCameraErrorDomain
                                                 code:NESimpleCameraErrorCodeCameraPermission
                                             userInfo:nil];
            [self passError:error];
        }
    }];
...
- (void)initialize
{
    if(!_session) {
        ...
        switch (self.position) {
            case NECameraPositionRear:
                if([self.class isRearCameraAvailable]) {
                    devicePosition = AVCaptureDevicePositionBack;
                } else {
                    devicePosition = AVCaptureDevicePositionFront;
                    _position = NECameraPositionFront;
                }
                break;
            case NECameraPositionFront:
                if([self.class isFrontCameraAvailable]) {
                    devicePosition = AVCaptureDevicePositionFront;
                } else {
                    devicePosition = AVCaptureDevicePositionBack;
                    _position = NECameraPositionRear;
                }
                break;
            default:
...
-(void)capture:(void (^)(NECameraController *camera, UIImage *image, NSDictionary *metadata, NSError *error))onCapture exactSeenImage:(BOOL)exactSeenImage animationBlock:(void (^)(AVCaptureVideoPreviewLayer *))animationBlock
{
    if(!self.session) {
        NSError *error = [NSError errorWithDomain:NESimpleCameraErrorDomain
                                    code:NESimpleCameraErrorCodeSession
                                userInfo:nil];
...
- (void)startRecordingWithOutputUrl:(NSURL *)url didRecord:(void (^)(NECameraController *camera, NSURL *outputFileUrl, NSError *error))completionBlock
{
    ...
    if(!self.videoEnabled) {
        NSError *error = [NSError errorWithDomain:NESimpleCameraErrorDomain
                                             code:NESimpleCameraErrorCodeVideoNotEnabled
                                         userInfo:nil];
        [self passError:error];
        return;
    }
    
    if(self.flash == NECameraFlashOn) {
...
- (BOOL)updateFlashMode:(NECameraFlash)cameraFlash
{
    ...
    if(cameraFlash == NECameraFlashOn) {
        flashMode = AVCaptureFlashModeOn;
    } else if(cameraFlash == NECameraFlashAuto) {
...
- (void)setMirror:(NECameraMirror)mirror
{
    _mirror = mirror;
    ...
    switch (mirror) {
        ...
        case NECameraMirrorAuto: {
            BOOL shouldMirror = (_position == NECameraPositionFront);
            if ([videoConnection isVideoMirroringSupported]) {
...
- (NECameraPosition)togglePosition
{
    ...
    if(self.position == NECameraPositionRear) {
        self.cameraPosition = NECameraPositionFront;
    } else {
        self.cameraPosition = NECameraPositionRear;
    }
    
    return self.position;
}

- (void)setCameraPosition:(NECameraPosition)cameraPosition
{
    if(_position == cameraPosition || !self.session) {
        return;
    }
    
    if(cameraPosition == NECameraPositionRear && ![self.class isRearCameraAvailable]) {
        return;
    }
    
    if(cameraPosition == NECameraPositionFront && ![self.class isFrontCameraAvailable]) {
        return;
    }
